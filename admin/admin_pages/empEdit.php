...
if(isset($_POST['opdater']))
    {
        ...
        $sqli = "UPDATE `employees` SET `first_name`='$editFirstName', `last_name`='$editLastName', `email`='$editEmail', `fk_role`='$editRole' WHERE id = '$empID'";
        $conn->query($sqli);
        ...
        if($_FILES['editFileToUpload']['error'] == 4) {
            ...
            echo 'beginning';
            $sqliImg = $conn->query("SELECT pictures.filename, picturecategory.filepath, employees.id 
                                     FROM employees
                                     INNER JOIN pictures
            ...
            $target_dir = '../img/'.$row['filepath'].'/'.$row['filename'].'';
		...
		if (move_uploaded_file($_FILES["editFileToUpload"]["tmp_name"], $target_file)) {
            ...
            echo 'file uploaded to folder';
                if ($fileDeleted == 1) 
                {
                        ...
                        if (file_exists($target_file)) {
                            if (unlink('../img/'.$row['filepath'].'/'.$row['filename'].'')) {
                            $unlinkComplete = 1;
                                                    ...
                                                    if($dbImgInsertComplete == 1)
                                                        {
                                                            ...
                                                            //Update product informaion in product databse
                                                                    $stmt = "UPDATE employees SET fk_img='$lastId' WHERE id='$prodId'";
                                                            $conn->query($stmt);
